...
const App = () => {
      ...
      const toolExecutor = async (name: string, args: any) => {
        ...
        switch (name) {
          ...
          case 'runPythonVisionAnalysis':
             ...
             return {
                 status: "success",
                 detected_objects: [args.defectClass],
                 confidence: 0.98,
  ...
  const confirmReturnReason = (reason: string) => {
      ...
      const prompt = `I need help with Order #${selectedOrderForReturn?.orderId}.
      Reason: ${reason}.
      Please help me process this request.`;
                 ...
                 <div key={item.id} className="flex gap-4 p-4 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 shadow-sm">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex justify-between items-end mt-2">
                        <span className="text-sm bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded text-slate-600 dark:text-slate-300">Qty: {item.quantity}</span>
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-50 font-sans transition-colors duration-300">
      ...
      <nav className="sticky top-0 z-40 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
            <div className="flex items-center gap-8">
                <span className="text-xl font-bold bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-cyan-500 cursor-pointer" onClick={() => setActiveTab('support')}>
                    AutoReturn AI
                </span>
                <div className="hidden md:flex items-center gap-1">
                    <button 
                        ...
                        className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                            activeTab === 'support' ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' : 'text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-100'
                        }`}
                    >
                        AI Support Agent
                    </button>
                    <button 
                        ...
                        className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                            activeTab === 'catalog' ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' : 'text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-100'
                        }`}
                    >
                        Catalog
                    </button>
                    <button 
                        ...
                        className={`px-4 py-2 rounded-full text-sm font-medium transition-all ${
                            activeTab === 'profile' ? 'bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-400' : 'text-slate-600 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-100'
                        }`}
                    >
                        My Profile
                    </button>
            ...
            <div className="flex items-center gap-4">
                <div 
                    ...
                    onClick={() => setActiveTab('profile')}
                >
                    <div className="text-sm font-medium group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">{user?.name}</div>
      ...
      <main className="max-w-7xl mx-auto px-4 py-8">
        ...
        {activeTab === 'support' && (
            <div className="max-w-4xl mx-auto h-[80vh] flex flex-col bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-xl overflow-hidden relative animate-in fade-in zoom-in-95 duration-300">
                {/* Agent Header */}
